...
$centroCosto= $obj2->get_centroCostoJefe($_SESSION['cod_epl']);
...
<body id="main">
							...
							<div class="modal-body2">
								<div class="label1">
									...
									<select name="centro_costo" class="centro_costo required">
										<option value="<?php echo $datos[0]['cod_cc2']?>"><?php echo $datos[0]['nom_cc2']?></option>
										<option value="">Seleccione...</option>
										<?php
											for($i=0; $i<count($centroCosto); $i++){
												
												if(@$centroCosto[$i]['codigo'] !=$datos[0]['cod_cc2'] && $datos[0]['nom_cc2'] != $centroCosto[$i]['area'] ){
												
													echo "<option value='".@$centroCosto[$i]['codigo']."' >".@$centroCosto[$i]['area']."</option>";
												}	
								...
								<?php 
									if($obj3->get_cargosID($datos[0]['cod_car']) == 1){
								?>
									<label class="radio">
										<input type="radio" name="cargoRequerido"  checked value="1">
										<select name="cargo" class="cargo">
												<option value="<?php echo $datos[0]['cod_car']?>"><?php echo $datos[0]['nom_car']?></option>											
												<option value="">Cargo</option>	
										</select>
									</label>
									<label class="radio">
										<input type="radio" name="cargoRequerido"  value="2">
										<input type="text"  class="span3" name="nuevo_cargo" value=""/> Escriba su cargo nuevo no existente
									</label>
								<?php }else{?>
									<label class="radio">
									...
									<label class="radio">
										<input type="radio" name="cargoRequerido" checked value="2">
										<input type="text"  class="span3" name="nuevo_cargo" value="<?php echo $datos[0]['cod_car']?>"/> Escriba su cargo nuevo no existente
									</label>
								<?php }?>
								...
								<?php }?>
								<label class="login_label control-label persona">Formulario de Sustentaci&oacute;n de la necesidad: <a href="#" id="show_frmSustentacion">Click</a><span id="mostrar_delete"><img src="../../img/delete.png" /></span><span id="mostrar_success"><img src="../../img/success.gif" /></span></label>
								<label class="login_label2 control-label">Persona Sugerida a Ocupar el Cargo:</label>
								<input class="span4" type="text" name="persona_sugerida" value="<?php echo @$datos[0]['per_sugerida']?>"/>
								<div style="clear:both"></div>
								<!--codigo del jefe-->
								<input type="hidden" name="cod_epl_jefe" value="<?php echo $_SESSION['cod_epl'] ?>" id="cod_epl" />
								<!--codigo del  formulario de la sustentacion-->
								<input type="hidden"  id="llenar" name="cod_form"  value="<?php echo @$datos[0]['cod_form']?>"/>
								<input type="hidden"  id="llenar1" name="nom_car_nuevo" value="<?php echo @$datos[0]['nom_car_nuevo']?>" />
								<input type="hidden"  id="llenar2" name="ubicacion2" value="<?php echo @$datos[0]['ubicacion']?>" />
								...
								<div class="btn-form2" ><input id="EDrequisicion_nuevo_cargoBtn" type="button" class="btn btn-primary" value="Enviar"/></div>
		...
		<div id="mostrar">
			<?php
				...
				include_once('ver_perfil_cargo.php');
			?>
		</div>
